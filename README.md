...
| `odoo_project_post_message` | Publica un mensaje o nota en el chatter de una tarea. Soporta `mention_partner_ids` para etiquetar/notificar contactos. |
| `odoo_project_read_messages` | Lee los mensajes y notas del chatter de una tarea (para revisar lo publicado antes de sumar más). |
| `odoo_message_delete` | Elimina un mensaje del chatter por ID (para corregir publicaciones). |
| `odoo_search_partner` | Busca contactos/usuarios (res.partner) por nombre, para usarlos como `mention_partner_ids`. |
